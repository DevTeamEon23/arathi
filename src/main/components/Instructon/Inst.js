...
const LearningActivityChart = loadable(() =>
  pMinDelay(import("./../Dashboard/Dashboard/LearningActivityChart"), 1000)
);
...
const Inst = () => {
  ...
  const getDataCounts = async () => {
    try {
      ...
      const url = new URL(
        "lms-service/data_counts_for_instructor"
      );
  ...
  const getUserActivity = async () => {
    try {
      ...
      const url = new URL(
        "lms-service/fetch_userpoints_by_userid_for_instructor"
      );
      url.search = new URLSearchParams(queryParams).toString();
      const response = await axios.get(url.toString(), {
        headers: {
  ...
  const getDeptCount = () => {
    ...
    axios
      .get(
        "lms-service/department_counts_for_instructor",
        config
      )
      .then((response) => {
  ...
  const getUserEnrolledCourses = async () => {
    try {
      ...
      const url = new URL(
        "lms-service/fetch_user_enrolled_course_data_for_instructor"
      );
      url.search = new URLSearchParams(queryParams).toString();
      const response = await axios.get(url.toString(), {
        headers: {
              ...
              <div className="card overflow-hidden">
                ...
                <div className="card-body pt-0">
                  <div className="d-flex align-items-center courses-chart flex-wrap">
                    ...
                    <span>
                      <small className="text-secondary">+15% </small>than last
                      year
                    </span>
                  </div>
